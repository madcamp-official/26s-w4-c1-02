...
```bash
pnpm typecheck && pnpm test        # 927개: core 707 · web 58 · worker 162
```
...
curl "http://localhost:3000/api/v1/bizinfo?view=closing-soon"                # 77 → 22건 (§4-1 실측)
curl "http://localhost:3000/api/v1/lib?view=closing-soon"                    # 남의 컬렉션 뷰는 안 걸린다 (경고만)
```
...
## 8. 07-28 아침에 판정된 것 — **Day 4 시작 시점의 상태**

> §0~§7 은 07-27 심야 기준이다. 이 절만 07-28 아침이다.
> 새 세션은 **§0 → §8 순으로 읽으면 지금 상태를 안다.**

### 8-1. 스케줄이 실제로 발화했다 — §4-2 가 닫혔다 ✅

기다려야만 알 수 있던 둘이 밤사이 실측됐다. 재현은 SQL 이다 (`runs` · `notification_log`).

| 무엇 | 실측 |
|---|---|
| 정기 수집 | **06:08~06:50 KST**. lib.seoul 30 · bizinfo 45 · k-startup 15, 전부 신규 0 |
| KST 00:05 뷰 평가 | **00:14 에 알림 9건** 발송. `closing-soon` 매칭 29건 |

gates.md G3 "스케줄 수집이 사람 손 없이 돈다" 를 체크해도 된다. **A 몫의 G3 는 이걸로 다 찼다.**

### 8-2. 429 문구가 실전에서 확인됐다 (§2-10 의 사후 검증) ✅

밤사이 wevity·k-startup 의 자가 치유가 막힌 키를 만났고, `runs.error_summary` 에 이렇게 남았다:

> 새 사이트를 읽어내는 기능이 지금 막혀 있어요. **시간이 지나도 저절로 풀리지 않는 종류라**, 설정을 한 번 봐주셔야 합니다.

어제였으면 "내일 다시 시도하면 됩니다" 가 나왔을 자리다. 단위 테스트가 아니라 **실제 로그**로 확인됐다.

### 8-3. 지금 막혀 있는 것 셋 🔴

| 무엇 | 상태 | 왜 중요한가 |
|---|---|---|
| **프로덕션 502** | `https://endpoint.madcamp-kaist.org` 가 502 | B 가 재배포 중이면 정상 과정. 아니면 G4 전체가 여기 걸린다 |
| **Gemini 키가 막혀 있다** | 잔액 소진 키로 되돌려둔 상태 | **자가 치유가 정지했다.** wevity 가 07-27 에 깨졌는데 스스로 못 고치는 중. 데모에 기능 ④ 가 들어가면 이건 🟢 가 아니라 🔴 다 |
| **실험 잔재 컬렉션 3개** | `contest` · `c-7ab3a820c3` · `c-bc4abdeaa7`(한양대, paused) | 실패 기록만 쌓고 있고 데모 화면에 그대로 나온다. 지울 거면 일회성 SQL (델타 10절 — **삭제 코드는 만들지 마라**) |

### 8-4. 오늘(Day 4)의 판정은 G4 다 — 급한 순서

1. 🔴 프로덕션 502 원인 확인 — 나머지 전부가 여기 걸려 있다
2. 🔴 **낯선 사람 통과 실험** — 설명 없이 로그인→URL 2개→표. **사람이 필요하니 미리 섭외해라**
3. 🔴 배포판 MCP 를 실제 커넥터에 붙여 "이번 주 마감 뭐 있어?" (로컬 JSON-RPC 는 07-27 통과)
4. 🔴 재부팅 테스트 — 설정은 다 됐고 실제로 껐다 켜본 적이 없다
5. 🟡 B1~B7 전수 점검 — 하나라도 위반이면 G4 미통과. 시간이 걸리니 먼저 시작해라
6. 🟡 Gemini 키 복구 (8-3 참조)
7. 🟢 실험 잔재 정리 · ISO 8601 시각 소실(§4-4) · 없는 뷰 이름 응답(§4-1 끝)
