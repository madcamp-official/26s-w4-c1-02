...
| [`jobs/heal.ts:107`](../apps/worker/src/jobs/heal.ts) | json 소스 치유에서 네트워크 관찰을 꺼 깨진 내부 API 재발견 경로가 막힘. **✅ 확인·수리 (07-28)** — 코드 경로로 확정: 네트워크 관찰(3단계)은 `skipBrowser` 에 묶여 있는데 치유가 `mode !== 'browser'` 로 json 까지 껐다. `healSkipsBrowser()` 로 분리 — html 만 끄고 json·browser 는 살린다 (행렬 테스트 3개). **실전 재현(내부 API 소스를 실제로 깨뜨리기)은 안 했다** — json 모드 실소스가 생기면 heal 데모 대본에 추가할 것 | ✅ |
