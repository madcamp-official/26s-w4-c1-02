...
//
// ── DNS 리바인딩 (TOCTOU) — 이제 fetch 경로는 접속 시점에도 막힌다 (ADR A41) ──
// 확인이 끝난 뒤 `fetch` 가 이름을 **다시** 푼다. 그 사이 답이 127.0.0.1 로 바뀌면(리바인딩)
// 확인한 주소와 실제로 붙는 주소가 달라진다. fetch 계열(html·json 수집 · 웹훅)은
// `guarded-dispatcher.ts` 가 **접속 직전 이름 풀기**에 이 판정을 한 번 더 끼워 그 틈을 닫았다.
// 이 함수는 여전히 앞단 검사로 남는다 — 두 겹은 의도된 것이다.
// **아직 남은 곳: browser 모드(Playwright)** 는 undici 를 안 타서 접속 시점 고정이 없다.
// 라우트 훅의 글자 판정(checkOutboundUrlSync)만 있으므로, 리바인딩은 브라우저 경로에 한해 열려 있다.
